...
public:
  /// c-tor
  BroadcastHeader (uint8_t e_start = 0, Time e_time = MilliSeconds(0), uint16_t source = 0, uint16_t e_dst_x = 0, uint16_t e_dst_y = 0, uint8_t instruction = 0, uint16_t hopCount = 0, uint8_t sequence = 0);
  ...
  // Fields
  void SetE_Start(uint8_t start)
  {
    e_start = start;
  }
  uint8_t GetE_Start() const
  {
    return e_start;
  }
  ...
  void SetE_Dst_X(uint16_t destination_x)
  {
    e_dst_x = destination_x;
  }
  uint16_t GetE_Dst_X() const
  {
    return e_dst_x;
  }

  void SetE_Dst_Y(uint16_t destination_y)
  {
    e_dst_y = destination_y;
  }
  uint16_t GetE_Dst_Y() const
  {
    return e_dst_y;
  }

  void SetInstruction(uint8_t inst)
  {
    instruction = inst;
  }
  ...
  void SetSequence (uint8_t number) 
  { 
    sequence = number; 
  }
...
public:
  /// c-tor
  UnicastHeader (uint8_t e_start = 0, Time e_time = MilliSeconds(0), uint16_t source = 0, uint16_t e_dst = 0, uint8_t instruction = 0, uint16_t hopCount = 0);
